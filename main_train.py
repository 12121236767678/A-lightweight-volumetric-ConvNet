...
from networks.UXNet_3D.newcnn_3d import UXNETWithShiftedPatchTokenization, ShiftedPatchTokenization
...
parser = argparse.ArgumentParser(description='3D LVCN: A lightweight volumetric ConvNet')
...
parser.add_argument('--dataset', type=str, default='MSD', required=True, help='Datasets: {feta, MSD, amos}, Fyi: You can add your dataset here')

## Input model & training hyperparameters
parser.add_argument('--network', type=str, default='3DLVCN', help='Network models: {TransBTS, nnFormer, UNETR, SwinUNETR, 3DLVCN}')
...
parser.add_argument('--max_iter', type=int, default=45000, help='Maximum iteration steps for training')
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_val):
            ...
            epoch_iterator_val.set_description(
                "Test (%d / %d Steps) (dice=%2.5f)" % (global_step, 10.0, dice)
            )
        dice_metric.reset()
    mean_dice_val = np.mean(dice_vals)
    writer.add_scalar('Test Segmentation Loss', mean_dice_val, global_step)
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (
            ...
            if dice_val > dice_val_best:
                ...
                torch.save(
                    model.state_dict(), os.path.join(root_dir, "best_metric_model_Liver_3DLVCN.pth")
                )
                print(
